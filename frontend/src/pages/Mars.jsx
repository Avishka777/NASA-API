          ...
          <div key={photo.id} className="mb-8">
            <div className='flex flex-col md:flex-row'>
              ...
              <div className='md:ml-10'>
                ...
                <p className="text-lg font-semibold text-zinc-700">Rover ID : {photo.camera.rover_id}</p>
                <p className="text-lg font-semibold text-zinc-700">Rover Name : {photo.rover.name}</p>
                <p className="text-lg font-semibold text-zinc-700">Rover Launch Date : {photo.rover.launch_date}</p>
                <p className="text-lg font-semibold text-zinc-700">Rover Landing Date : {photo.rover.landing_date}</p>
                <p className="text-lg font-semibold text-zinc-700 mb-5">Rover Status: {photo.rover.status}</p>
                <span className='text-3xl font-semibold'>Camera Details</span>
                <hr className='my-2 border-2 border-blue-500 font-bold' />
                <p className="text-lg font-semibold text-zinc-700">Camera Short Name : {photo.camera.name}</p>
                <p className="text-lg font-semibold text-zinc-700 mb-5">Camera Full Name : {photo.camera.full_name}</p>
                <span className='text-3xl font-semibold'>Other Details</span>
                <hr className='my-2 border-2 border-blue-500 font-bold' />
                <p className="text-lg font-semibold text-zinc-700">Earth Date: {photo.earth_date}</p>
                <p className=' text-lg font-serif text-sky-700'>View HD Image : <a href={photo.img_src} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">{photo.img_src}</a></p>
              </div>
            </div>
          </div>
        ))}
